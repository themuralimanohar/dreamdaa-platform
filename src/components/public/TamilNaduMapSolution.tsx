'use client'

import { useEffect, useRef, useState, useCallback } from 'react'
import { X, School, Users, TrendingUp } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { 
  tamilNaduDistricts, 
  getDistrictColor, 
  getFundingStatusText, 
  getCoveragePercentage,
  type DistrictData 
} from '@/lib/tamilnadu-districts-data'

interface MapPopupProps {
  district: DistrictData | null
  onClose: () => void
}

function MapPopup({ district, onClose }: MapPopupProps) {
  if (!district) return null
  const coveragePercentage = getCoveragePercentage(district)

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-2xl border border-gray-200 p-6 w-96 mx-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-xl text-gray-900">{district.name} District</h3>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full">
            <X className="w-5 h-5 text-gray-500" />
          </button>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <div 
            className="w-4 h-4 rounded-full"
            style={{ backgroundColor: getDistrictColor(district.fundingStatus) }}
          />
          <span className="font-medium text-gray-700">
            {getFundingStatusText(district.fundingStatus)} - {coveragePercentage}% Coverage
          </span>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <School className="w-6 h-6 text-blue-600 mx-auto mb-2" />
            <div className="text-2xl font-bold text-blue-900">{district.totalSchools}</div>
            <div className="text-sm text-blue-600">Total Schools</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <Users className="w-6 h-6 text-green-600 mx-auto mb-2" />
            <div className="text-2xl font-bold text-green-900">{district.coveredSchools}</div>
            <div className="text-sm text-green-600">Covered</div>
          </div>
        </div>

        {district.availableForSponsorship > 0 && (
          <div className="bg-orange-50 rounded-lg p-4 mb-4">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp className="w-5 h-5 text-orange-600" />
              <span className="font-semibold text-orange-900">Need Sponsorship</span>
            </div>
            <div className="text-2xl font-bold text-orange-900">{district.availableForSponsorship}</div>
            <div className="text-xs text-orange-600">Schools need support</div>
          </div>
        )}

        <Button 
          className="w-full bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white font-semibold"
          onClick={() => window.location.href = `/donate?district=${district.id}&type=school`}
        >
          Sponsor Schools in {district.name}
        </Button>
      </div>
    </div>
  )
}

export default function TamilNaduMapSolution() {
  const [selectedDistrict, setSelectedDistrict] = useState<DistrictData | null>(null)
  const [mapStatus, setMapStatus] = useState<'loading' | 'loaded' | 'error'>('loading')
  const [debugStatus, setDebugStatus] = useState<string>('Initializing...')

  // Use callback ref instead of useRef to ensure we get notified when element is ready
  const mapRef = useCallback((node: HTMLDivElement | null) => {
    if (node) {
      console.log('mapRef callback fired - DOM element is ready!', node)
      setDebugStatus('DOM element ready via callback!')
      initializeMap(node)
    }
  }, [])

  const panelRef = useRef<HTMLDivElement>(null)

  const initializeMap = (mapElement: HTMLDivElement) => {
    console.log('initializeMap called with element:', mapElement)
    setDebugStatus('Starting map initialization...')

    if (typeof window === 'undefined') {
      setDebugStatus('Window not available')
      return
    }

    const districtMap = new Map()
    tamilNaduDistricts.forEach(district => {
      districtMap.set(district.name, district)
    })

    let mounted = true

    const loadLeaflet = () => {
      setDebugStatus('Loading Leaflet CSS and JS...')
      
      // Load Leaflet CSS
      if (!document.querySelector('link[href*="leaflet.css"]')) {
        const css = document.createElement('link')
        css.rel = 'stylesheet'
        css.href = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'
        document.head.appendChild(css)
        console.log('Leaflet CSS loaded')
      }

      // Load Leaflet JS
      if (!(window as any).L) {
        setDebugStatus('Loading Leaflet JavaScript...')
        const script = document.createElement('script')
        script.src = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'
        script.onload = () => {
          console.log('Leaflet JS loaded')
          createMap()
        }
        script.onerror = () => {
          console.error('Failed to load Leaflet JS')
          setDebugStatus('Error: Failed to load Leaflet JS')
          if (mounted) setMapStatus('error')
        }
        document.head.appendChild(script)
      } else {
        console.log('Leaflet already available')
        createMap()
      }
    }

    const createMap = () => {
      if (!mounted) return
      
      try {
        console.log('Creating map with element:', mapElement)
        setDebugStatus('Creating Leaflet map...')
        
        const L = (window as any).L
        const map = L.map(mapElement, {
          center: [11.1271, 78.6569],
          zoom: 7,
          zoomControl: true,
          attributionControl: false
        })

        console.log('Map instance created')
        setDebugStatus('Map created, adding tiles...')

        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
          attribution: '© OpenStreetMap'
        }).addTo(map)

        console.log('Tiles added')
        setDebugStatus('Fetching Tamil Nadu GeoJSON...')

        fetch('https://cdn.jsdelivr.net/gh/udit-001/india-maps-data@main/geojson/states/tamil-nadu.geojson')
          .then(response => {
            console.log('GeoJSON fetch response:', response.status)
            setDebugStatus(`GeoJSON response: ${response.status}`)
            
            if (!response.ok) {
              throw new Error(`HTTP ${response.status}`)
            }
            return response.json()
          })
          .then(geojson => {
            if (!mounted) return
            
            console.log('GeoJSON data loaded:', geojson.features?.length, 'features')
            setDebugStatus(`Processing ${geojson.features?.length} districts...`)

            const geoLayer = L.geoJSON(geojson, {
              style: (feature: any) => {
                const props = feature.properties || {}
                const name = (props.district || props.DIST_NAME || props.District || props.NAME_2 || 'Unknown').toString().trim()
                
                let district = districtMap.get(name)
                if (!district) {
                  for (const d of tamilNaduDistricts) {
                    if (d.name.toLowerCase().includes(name.toLowerCase()) || 
                        name.toLowerCase().includes(d.name.toLowerCase())) {
                      district = d
                      break
                    }
                  }
                }

                return {
                  fillColor: district ? getDistrictColor(district.fundingStatus) : '#3b82f6',
                  weight: 2,
                  opacity: 1,
                  color: 'white',
                  fillOpacity: 0.8
                }
              },
              onEachFeature: (feature: any, layer: any) => {
                const props = feature.properties || {}
                const name = (props.district || props.DIST_NAME || props.District || props.NAME_2 || 'Unknown').toString().trim()
                
                let district = districtMap.get(name)
                if (!district) {
                  for (const d of tamilNaduDistricts) {
                    if (d.name.toLowerCase().includes(name.toLowerCase()) || 
                        name.toLowerCase().includes(d.name.toLowerCase())) {
                      district = d
                      break
                    }
                  }
                }

                layer.on('mouseover', (e: any) => {
                  e.target.setStyle({ weight: 3, color: '#666' })
                })
                
                layer.on('mouseout', (e: any) => {
                  e.target.setStyle({ weight: 2, color: 'white' })
                })

                layer.on('click', (e: any) => {
                  if (district) {
                    setSelectedDistrict(district)
                    
                    if (panelRef.current) {
                      const coverage = getCoveragePercentage(district)
                      const status = getFundingStatusText(district.fundingStatus)
                      
                      panelRef.current.innerHTML = `
                        <h3 class="text-xl font-bold text-gray-900 mb-2">${district.name}</h3>
                        <div class="text-sm text-gray-600 mb-4">Tamil Nadu District</div>
                        <div class="p-3 rounded-lg bg-blue-50 border-l-4 border-blue-500 mb-4">
                          <div class="font-semibold">${status}</div>
                          <div class="text-2xl font-bold">${coverage}%</div>
                          <div class="text-sm">Schools: ${district.coveredSchools}/${district.totalSchools}</div>
                          ${district.availableForSponsorship > 0 ? 
                            `<div class="text-sm mt-1">${district.availableForSponsorship} schools need sponsorship</div>` :
                            '<div class="text-sm mt-1 text-green-600">All schools funded! 🎉</div>'
                          }
                        </div>
                        <div class="text-xs text-gray-500">Click for detailed sponsorship options</div>
                      `
                    }
                  }

                  e.target.bindPopup(`<strong>${name}</strong>${district ? `<br/>${getFundingStatusText(district.fundingStatus)} - ${getCoveragePercentage(district)}% covered` : ''}`).openPopup(e.latlng)
                })
              }
            }).addTo(map)

            map.fitBounds(geoLayer.getBounds())
            
            if (mounted) {
              setMapStatus('loaded')
              setDebugStatus('SUCCESS: Tamil Nadu map loaded completely!')
              console.log('Map loading complete!')
            }
          })
          .catch(err => {
            console.error('GeoJSON error:', err)
            setDebugStatus(`GeoJSON error: ${err.message}`)
            if (mounted) setMapStatus('error')
          })

      } catch (error) {
        console.error('Map creation error:', error)
        setDebugStatus(`Map creation error: ${error}`)
        if (mounted) setMapStatus('error')
      }
    }

    // Cleanup function
    const cleanup = () => {
      mounted = false
    }

    loadLeaflet()
    return cleanup
  }

  if (mapStatus === 'loading') {
    return (
      <div className="w-full h-96 lg:h-[600px] rounded-xl bg-blue-50 flex items-center justify-center border">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-blue-700 font-semibold">Loading Tamil Nadu Map...</p>
          <p className="text-blue-600 text-sm mt-1">Loading real district boundaries</p>
          <div className="mt-4 text-xs text-blue-600 font-mono bg-blue-100 p-3 rounded max-w-sm">
            Debug: {debugStatus}
          </div>
        </div>
      </div>
    )
  }

  if (mapStatus === 'error') {
    return (
      <div className="w-full h-96 lg:h-[600px] rounded-xl bg-red-50 border border-red-200 flex items-center justify-center">
        <div className="text-center p-6">
          <h3 className="text-red-700 font-semibold mb-2">Map Loading Failed</h3>
          <p className="text-red-600 text-sm mb-4">Unable to load Tamil Nadu map</p>
          <div className="text-xs text-red-600 font-mono bg-red-100 p-3 rounded mb-4 max-w-sm">
            Debug: {debugStatus}
          </div>
          <Button 
            onClick={() => window.location.reload()}
            className="bg-red-600 hover:bg-red-700 text-white"
          >
            Try Again
          </Button>
        </div>
      </div>
    )
  }

  return (
    <div className="relative">
      {/* Title */}
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-gray-900 mb-4">
          Tamil Nadu District Map
        </h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Interactive map with real district boundaries. Click on any district to explore school coverage details.
        </p>
      </div>

      {/* Map Container */}
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-3" style={{ minHeight: '600px' }}>
          {/* Map */}
          <div className="lg:col-span-2">
            <div 
              ref={mapRef}
              className="w-full h-96 lg:h-[600px]"
              style={{ minHeight: '600px' }}
            />
          </div>
          
          {/* Side Panel */}
          <div className="border-l border-gray-200 bg-gray-50 p-4 overflow-auto">
            <div ref={panelRef} className="space-y-4">
              <h3 className="text-xl font-bold text-gray-900">TN District Explorer</h3>
              <div className="text-sm text-gray-600">Click a district to see school coverage details</div>
              <p className="text-sm text-gray-700">
                This map shows real Tamil Nadu district boundaries with school funding status. 
                Each district is colored based on coverage percentage.
              </p>
              
              {/* Legend */}
              <div className="space-y-2">
                <div className="text-sm font-semibold text-gray-700">Legend:</div>
                <div className="space-y-1 text-xs">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-red-500 rounded"></div>
                    <span>Needs Funding (0-40%)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-orange-500 rounded"></div>
                    <span>Partially Funded (41-70%)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-blue-500 rounded"></div>
                    <span>Well Funded (71-99%)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-green-500 rounded"></div>
                    <span>Fully Funded (100%)</span>
                  </div>
                </div>
              </div>
              
              <div className="text-xs text-gray-500">
                Tip: Ctrl/Cmd + scroll to zoom the map
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* District Detail Popup */}
      <MapPopup
        district={selectedDistrict}
        onClose={() => setSelectedDistrict(null)}
      />

      {/* Instructions */}
      <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
        <p className="text-green-800 text-sm">
          🗺️ <strong>Real Tamil Nadu Map:</strong> This map uses actual district boundaries from government data. Click districts for detailed sponsorship information.
        </p>
      </div>
    </div>
  )
}